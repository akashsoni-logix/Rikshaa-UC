...
class Alerts extends Component {
  ...
  handleNotificationClick = (alert) => {
    ...
    if (user.success) {
      this.props
        .markOneNotificationRead(user.data.id, alert.id, user.data.auth_token)
        .then((response) => {
        ...
        <div className="block-content block-content-full mb-100">
          {alerts.length ? (
            <React.Fragment>
              ...
              >
                {localStorage.getItem("markAllAlertReadText")}{" "}
                <i className="si si-check ml-1" />
            ...
            <div className="text-center mt-50 font-w600 text-muted pt-100">
              <div className="pb-10">
                <i
                  ...
                  style={{ fontSize: "10rem", color: "#eee" }}
                />
              </div>
              <p style={{ color: "#ccc" }}>
                {localStorage.getItem("noNewAlertsText")}
              </p>
            </div>
          )}
        </div>
